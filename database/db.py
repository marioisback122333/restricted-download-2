...
class Database:
    ...
    async def add_user(self, id, name):
        ...
        logger.info(f"New user added to DB: {id}")
    ...
    # --------------------------------------------------------
    # Session Support
    # WARNING: Session strings are stored in plain text.
    # For production use, consider encrypting session strings
    ...
    async def set_dump_chat(self, id, chat_id):
        """Set or clear the dump chat. Pass None to clear."""
        if chat_id is None:
    ...
    async def add_traffic(self, id):
        ...
        # If premium, don't count against limits
        if user.get('is_premium'):
            return
