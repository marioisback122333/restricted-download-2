...
async def my_plan(client: Client, message: Message):
    ...
    if not user_data:
        return await message.reply_text("❌ User data not found. Please /start first.")
...
async def myplan_back_callback(client: Client, callback_query: CallbackQuery):
    """
    Re-render myplan view from callback. We fetch the user_id from callback_query
    instead of message.from_user (which may not be set on edited messages).
    """
    ...
    if is_premium:
        ...
        plan_text = (
            ...
            f"<b>📅 Expiry:</b> {expiry_text}\n\n"
            f"<b>♾️ Daily Tokens:</b> Unlimited\n"
            f"<b>📊 Total Saves:</b> <code>{total_saves}</code>"
        )
    else:
        daily_limit = 10
        tokens_left = max(0, daily_limit - daily_usage)
